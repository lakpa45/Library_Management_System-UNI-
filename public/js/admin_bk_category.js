...
function render() {
  ...
  const filtered = categories.filter((category) => {
    ...
    const description = (category.desc || "").toLowerCase();
  ...
  if (filtered.length === 0) {
    ...
    filtered.forEach((category) => {
      grid.appendChild(buildCard(category));
    });
  }
  ...
  card.innerHTML = `
                <div>
                    ...
                    <p class="text-sm text-[#4A5A50] leading-snug">
                        ${escapeHTML(category.desc || "No description yet.")}
                    </p>
                </div>
...
function openModal(category = null) {
  ...
  document.getElementById("categoryDesc").value = category ? category.desc : "";
  document.getElementById("categoryCount").value = category
    ? category.count
    : 0;
  document.getElementById("categoryColor").value = category
    ? category.color
    : "#1B4332";
...
categoryForm.addEventListener("submit", (event) => {
  ...
  const count = Number(document.getElementById("categoryCount").value) || 0;
...
document.getElementById("confirmDeleteBtn").addEventListener("click", () => {
  ...
  categories = categories.filter((category) => category.id !== deleteTargetId);
  saveCategories();
  closeDeleteModal();
  render();
});
